...
// Le stanze dei Boss (espansione "M'Guf-yn Returns") hanno una pavimentazione
// dedicata, diversa da quella del livello normale corrispondente.
const BOSS_TILE_SETS_BY_LEVEL = {
  3:  'cobblestones', // Minotauro
  6:  'ritual-floor',  // Lich
  9:  'sand',          // Insettoide
  12: 'marble',        // M'Guf-yn
};
// Set di piastrelle da usare per lo stato di gioco attuale: normale o Boss.
function currentTileBiome(){
  if(state.levelPhase === 'boss' && BOSS_TILE_SETS_BY_LEVEL[state.level]){
    return BOSS_TILE_SETS_BY_LEVEL[state.level];
  }
  return tileBiomeForLevel(state.level);
}
...
function preloadAllGameImages(){
  ...
  [...TILE_SETS_BY_LEVEL_CYCLE, ...Object.values(BOSS_TILE_SETS_BY_LEVEL)].forEach(biome=>{
