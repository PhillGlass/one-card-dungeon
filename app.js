  ...
  for (const ca of cornersA) {
    for (const cb of cornersB) {
      ...
      for (const [wx, wy] of blockingTiles) {
        ...
        if (segRectIntersect(ca, cb, wx, wy)) {
          blocked = true;
          break;
        }
      }
...
const ICONS = {
  close:
    '<svg class="icon icon-lg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 7l10 10M17 7 7 17"/></svg>',
  ...
  arrowRight:
    '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 12h16"/><path d="m13 5 7 7-7 7"/></svg>',
  sparkle:
    '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3c0 4.2-1.1 6.3-3.1 8.3S3 14 3 14s3.8 1.5 5.9 3.6S12 21 12 21s0-4.2 2.1-6.4S21 12 21 12s-3.8-1.5-5.9-3.6S12 3 12 3z"/></svg>',
};
...
// Immagine mostrata nella modal di Vittoria (fallback 👑 se manca).
const SCEPTRE_IMG = "mguf-yn_sceptre.png";
...
function preloadImg(relPath) {
  ...
  const im = new Image();
  im.onload = () => {
    imgStatus[relPath] = true;
    queueImgRerender();
  };
  im.onerror = () => {
    imgStatus[relPath] = false;
  };
...
function preloadAllGameImages() {
  Object.values(MONSTER_TABLE).forEach((m) => {
    if (m.img) preloadImg(`enemies/${m.img}`);
  });
  Object.values(BOSS_TABLE).forEach((b) => {
    if (b.img) preloadImg(`bosses/${b.img}`);
  });
  Object.values(CLASSES).forEach((c) => {
    if (c.img) preloadImg(`classes/${c.img}`);
  });
  Object.values(ITEM_CARDS).forEach((c) => {
    if (c.img) preloadImg(`cards/${c.img}`);
  });
  preloadImg(`items/${CHEST_IMG}`);
  preloadImg(`items/${SCEPTRE_IMG}`);
  TILE_SETS_BY_LEVEL_CYCLE.forEach((biome) => {
    for (let y = 0; y < GRID; y++)
      for (let x = 0; x < GRID; x++) preloadImg(tileCellPath(biome, x, y));
  });
}
...
const CLASSES = {
  none: {
    name: "Classico",
    icon: "🗡️",
    img: "class_classic.png",
    desc: "Nessuna abilità speciale — il gioco base, senza varianti.",
  },
  paladin: {
    name: "Paladino",
    icon: "🛡️",
    img: "class_paladin.png",
    desc: "Una volta per Livello, puoi mantenere il valore di un dado Energia dal turno precedente invece di rilanciarlo.",
  },
  barbarian: {
    name: "Barbaro",
    icon: "🪓",
    img: "class_barbarian.png",
    desc: "Quando sei a 1 Salute, puoi rilanciare tutti i dadi Energia (una volta per turno, senza limite di livello).",
  },
  ranger: {
    name: "Ranger",
    icon: "🏹",
    img: "class_ranger.png",
    desc: "Una volta per Livello, puoi assegnare un dado alla Gittata invece che alla Velocità.",
  },
  wizard: {
    name: "Mago",
    icon: "🔮",
    img: "class_wizard.png",
    desc: "Una volta per Livello, puoi rilanciare tutti e tre i dadi Energia.",
  },
  necromancer: {
    name: "Negromante",
    icon: "🧛",
    img: "class_necromancer.png",
    expansion: "mguf_yn_returns",
...
const ITEM_CARDS = {
  wyrm_speed: {
    name: "Estratto di Wyrm Spinato",
    icon: "🐉",
    img: "card_wyrm-speed.png",
    expansion: "item_cards",
    ...
    use() {
      ...
      discardCard("wyrm_speed");
      render();
    },
  },
  halve_damage: {
    name: "Dimezza i Danni Subiti",
    icon: "🩹",
    img: "card_halve-damage.png",
    expansion: "item_cards",
    ...
    use() {
      ...
      discardCard("halve_damage");
      render();
    },
  },
  flame_of_fate: {
    name: "Fiamma del Fato",
    icon: "🔥",
    img: "card_flame-of-fate.png",
    expansion: "item_cards",
    desc: "Rilancia uno o più dadi Energia non ancora assegnati e tieni il nuovo risultato.",
    usable: () =>
      state.phase === "assign" &&
      !state.animating &&
      state.dice.some((d) => !d.target),
    use() {
      ...
      state.flameFateMode = true;
      state.flameFateSelected = [];
      render();
    },
  },
  rope_pick: {
    name: "Corda e Piccozza",
    icon: "⛏️",
    img: "card_rope-pick.png",
    expansion: "item_cards",
    desc: "Durante i tuoi attacchi standard di questo turno: +2 Attacco e +2 Gittata.",
    usable: () => state.phase === "act" && !state.animating,
    use() {
      state.itemBonus.atk += 2;
      state.itemBonus.range += 2;
      log(
        "⛏️ Corda e Piccozza: +2 Attacco, +2 Gittata per i tuoi attacchi standard.",
      );
      discardCard("rope_pick");
      render();
    },
  },
  gorgon_head: {
    name: "Testa della Gorgona",
    icon: "🐍",
    img: "card_gorgon-head.png",
    expansion: "item_cards",
  ...
  black_powder: {
    ...
    use() {
      ...
      log(
        "💥 Polvere Nera armata: il prossimo colpo si propagherà a tutti i mostri.",
      );
      discardCard("black_powder");
      render();
    },
  },
  dragon_eye: {
...
function itemAtkBonus() {
  return (state.itemBonus && state.itemBonus.atk) || 0;
}
function itemRangeBonus() {
  return (state.itemBonus && state.itemBonus.range) || 0;
}
function itemSpeedBonus() {
  return (state.itemBonus && state.itemBonus.speed) || 0;
}
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
window.__ocdCache = window.__ocdCache || {
  save: null,
  records: {},
  settings: { expansions: {} },
};
...
function modeKeyFor(expansions) {
  return expansions && expansions.includes("mguf_yn_returns")
    ? "mguf_yn_returns"
    : "vanilla";
}
...
function levelRank(level, phase) {
  return level * 10 + (phase === "boss" ? 5 : 0);
}
...
function finishRun() {
  ...
  const usesItemCards =
    state.expansions && state.expansions.includes("item_cards");
  if (!usesItemCards) {
...
function loadActiveExpansions() {
  return (
    (window.__ocdCache.settings && window.__ocdCache.settings.expansions) || {}
  );
}
function setExpansionActive(id, active) {
  if (!window.__ocdCache.settings)
    window.__ocdCache.settings = { expansions: {} };
  if (!window.__ocdCache.settings.expansions)
    window.__ocdCache.settings.expansions = {};
...
function startRun(cls) {
  ...
  state = {
    ...
    maxHp: 6,
    dice: [],
    points: { speed: 0, atk: 0, def: 0, range: 0 },
    spent: { speed: 0, atk: 0 },
    phase: "roll",
    player: { x: 0, y: 0 },
    walls: [],
    entryStair: { x: 0, y: 0 },
    exitStair: { x: GRID - 1, y: GRID - 1 },
    monsters: [],
    chest: null,
    loot: 0,
    lootUsedStat: null,
    ...
    itemCards: expansions.includes("item_cards")
      ? dealItemCards(expansions)
      : [],
...
function totalRange() {
  return state.skills.range + (state.points.range || 0);
}
...
function makeWalls(level, forbiddenExtra) {
  ...
  const allCells = [];
  for (let y = 0; y < GRID; y++)
    for (let x = 0; x < GRID; x++) allCells.push([x, y]);
...
function spawnNormalLevel(lvl) {
  ...
  for (let i = 0; i < t.count; i++) {
    ...
    do {
      ...
      (used.has(k) ||
        wallSet.has(k) ||
        chebyshev(x, y, state.player.x, state.player.y) < 2) &&
      tries < 200
    );
    used.add(k);
    monsters.push({
...
function spawnBossLevel(lvl) {
  ...
  const cx = Math.floor(GRID / 2),
    cy = Math.floor(GRID / 2); // (2,2) su griglia 5x5
  ...
  const forbidden = new Set([
    key(0, 0),
    key(state.exitStair.x, state.exitStair.y),
    key(cx, cy),
  ]);
...
function spawnLevel(lvl) {
  ...
  if (
    state.expansions.includes("mguf_yn_returns") &&
  ...
  state.lootUsedStat = null;
  state.lootDieActive = false;
...
function wallSetOf() {
  ...
  if (state.chest && !state.chest.opened)
    s.add(key(state.chest.x, state.chest.y));
  return s;
}

function totalStat(k) {
  return state.skills[k] + state.points[k];
}
...
function playerRangeTo(mx, my) {
  ...
  for (const m of state.monsters) {
    if (m.alive && !(m.x === mx && m.y === my)) blocked.add(key(m.x, m.y));
  }
...
function playerLoSTo(mx, my) {
  ...
  if (
    state.chest &&
    !state.chest.opened &&
    !(state.chest.x === mx && state.chest.y === my)
  ) {
    blocking.push([state.chest.x, state.chest.y]);
  }
  for (const m of state.monsters) {
    if (m.alive && !(m.x === mx && m.y === my)) blocking.push([m.x, m.y]);
  }
...
function animateDiceRoll(diceObjs, lockedIndices) {
  ...
  return new Promise((resolve) => {
    ...
    const els = diceObjs.map((d, i) => {
      ...
      el.className =
        "die" + (locked ? " settled" : " rolling") + (d.isD12 ? " d12" : "");
    ...
    const iv = setInterval(() => {
      els.forEach((el, i) => {
        if (!lockedIndices.includes(i)) el.textContent = randFor(diceObjs[i]);
      });
      ticks++;
      if (ticks >= 7) {
        clearInterval(iv);
        els.forEach((el, i) => {
          el.textContent = diceObjs[i].value;
...
async function rollKeepingDie(kept) {
  ...
  state.animating = false;
  log(
    `🛡️ Abilità Paladino: mantieni un dado da ${kept}. Nuovi dadi: ${state.dice.map((d) => d.value).join(", ")}`,
  );
  render();
}
...
async function wizardReroll() {
  ...
  state.animating = false;
  log(
    `🔮 Abilità Mago: rilanci tutti i dadi. Nuovo tiro: ${state.dice.map((d) => d.value).join(", ")}`,
  );
  render();
}
...
async function barbarianReroll() {
  ...
  state.animating = false;
  log(
    `🪓 Abilità Barbaro (1 Salute): rilanci tutti i dadi. Nuovo tiro: ${state.dice.map((d) => d.value).join(", ")}`,
  );
  render();
}
...
function clericBoost() {
  ...
  log(
    `✨ Abilità Chierico: dadi in tris, aumentati di 2 (max 6). Nuovo tiro: ${state.dice.map((d) => d.value).join(", ")}`,
  );
  render();
}
...
function thiefBoost() {
  ...
  log(
    `🗝️ Abilità Ladro: tutti i dadi +1. Nuovo tiro: ${state.dice.map((d) => d.value).join(", ")}`,
  );
  render();
}
...
async function confirmFlameFate() {
  ...
  const lockedIndices = state.dice
    .map((d, i) => i)
    .filter((i) => !selected.includes(i));
  await animateDiceRoll(diceForAnim, lockedIndices);
  selected.forEach((i) => {
    state.dice[i].value = diceForAnim[i].value;
  });
  log(
    `🔥 Fiamma del Fato: rilanci ${selected.length} dado/i. Nuovo tiro: ${state.dice.map((d) => d.value).join(", ")}`,
  );
  state.flameFateMode = false;
  state.flameFateSelected = [];
...
function assignTo(stat) {
  ...
  const knightDouble =
    state.class === "knight" &&
    !state.abilityUsed.knight &&
    ((stat === "speed" && speedTaken) ||
      (stat === "atk" && atkTaken) ||
      (stat === "def" && defTaken));
  ...
  if (stat === "range") {
    ...
    log(
      `🏹 Abilità Ranger: assegni un dado (${d.value}) alla Gittata invece che alla Velocità.`,
    );
  }
  ...
  if (knightDouble) {
    ...
    log(
      `🐴 Abilità Cavaliere: secondo dado (${d.value}) sommato sulla stessa caratteristica.`,
    );
  }
  maybeAdvanceToAct(false);
  render();
...
function maybeAdvanceToAct(force) {
  ...
  log(
    `Energia assegnata — Velocità ${totalStat("speed")}, Attacco ${totalStat("atk")}, Difesa ${totalStat("def")}${state.points.range ? `, Gittata +${state.points.range}` : ""}`,
  );
}
...
function cellInfo(x, y) {
  ...
  if (occ === "monster") {
    ...
    const inRange =
      dist !== undefined && dist <= totalStat("range") + itemRangeBonus(); // fixed, unless boosted by the Ranger ability or item cards
  ...
  if (occ === "chest") {
    const dist = playerRangeTo(x, y);
    const inRange =
      dist !== undefined && dist <= totalStat("range") + itemRangeBonus();
    const los = playerLoSTo(x, y);
    const remAtk = totalStat("atk") + itemAtkBonus() - state.spent.atk;
  ...
  if (dist !== undefined && dist <= remSpeed && dist > 0)
    return { type: "move", dist };
  return { type: null };
}
...
function attackSelected() {
  ...
  if (m.hp <= 0) {
    ...
    log(
      `Colpisci ${m.name} ${m.icon} (−${m.def}⚡ Attacco): resta ${m.hp}/${m.maxHp} Salute.`,
    );
  }
  ...
  if (state.blackPowderArmed) {
    ...
    if (hitCount)
      log(`💥 Polvere Nera: il colpo si propaga ad altri ${hitCount} mostri.`);
  }
  render();
}
...
function openChest() {
  ...
  log(
    `🎁 Apri la Cassa del Tesoro (−${state.chest.roll}⚡ Attacco): ottieni ${state.chest.roll} punti Bottino.`,
  );
  state.selectedChest = false;
  render();
}
...
function necromancerStrike() {
  ...
  if (target.hp <= 0) {
    target.alive = false;
    log(
      `🧛 Abilità Negromante: −1 Salute, abbatti ${target.name} ${target.icon} ignorandone la Difesa!`,
    );
  } else {
    log(
      `🧛 Abilità Negromante: −1 Salute per infliggere 1 danno a ${target.name} ${target.icon} (ignora la Difesa).`,
    );
  }
  render();
  if (state.hp <= 0) {
...
async function monsterMovementPhase() {
  ...
  for (const m of order) {
    ...
    const effSpeed =
      state.monsterSpeedOverride != null ? state.monsterSpeedOverride : m.speed;
    ...
    for (const o of state.monsters)
      if (o !== m && o.alive) stopBlocked.add(key(o.x, o.y));
    ...
    const attackableNow = candidates.filter(
      (c) => c.reachable && c.rangeFromAdv <= m.range && c.los,
    );
    if (attackableNow.length) {
      attackableNow.sort(
        (a, b) => b.rangeFromAdv - a.rangeFromAdv || a.moveCost - b.moveCost,
      );
      ...
      const pool = reachableCandidates.length
        ? reachableCandidates
        : candidates;
      const anyLos = pool.filter((c) => c.los);
      if (anyLos.length) {
        anyLos.sort(
          (a, b) => a.rangeFromAdv - b.rangeFromAdv || a.moveCost - b.moveCost,
        );
        target = anyLos[0];
      } else if (pool.length) {
        pool.sort(
...
async function monsterAttackPhase() {
  ...
  if (attackers.length === 0) {
    ...
    log(
      `I mostri attaccano (Attacco tot. ${totalAtk} vs Difesa ${totalDef}): il tuo scudo regge, 0 danni.`,
    );
    spawnFloatText(state.player.x, state.player.y, "Miss", "miss");
  } else {
    state.hp = Math.max(0, state.hp - dmg);
    log(
      `I mostri attaccano (${totalAtk} Attacco vs ${totalDef} Difesa): subisci ${dmg} danni${halved ? " (dimezzati)" : ""}. Salute: ${state.hp}/${state.maxHp}`,
    );
...
async function endTurn() {
  ...
  if (state.monsters.every((m) => !m.alive)) {
    render();
    onLevelClear();
    return;
  }
...
function showClassSelect() {
  const bg = document.getElementById("modalBg"),
    m = document.getElementById("modalContent");
  const active = loadActiveExpansions();
  const visibleClasses = Object.entries(CLASSES).filter(
    ([id, c]) => !c.expansion || active[c.expansion],
  );
  m.innerHTML = `
  ...
  m.querySelector("#classCloseBtn").onclick = () => {
    bg.classList.add("hidden");
  };
...
function showItemCards() {
  ...
  m.querySelector("#cardsCloseBtn").onclick = () => {
    bg.classList.add("hidden");
  };
  bg.classList.remove("hidden");
}
...
function showGameOver() {
  ...
  const bg = document.getElementById("modalBg"),
    m = document.getElementById("modalContent");
  const levelLabel =
    state.levelPhase === "boss" ? `${state.level} · Boss` : `${state.level}`;
  ...
  bg.classList.remove("hidden");
  m.querySelector("#retryBtn").onclick = () => {
    bg.classList.add("hidden");
    showSplash();
  };
}
...
function showVictory() {
  ...
  const bg = document.getElementById("modalBg"),
    m = document.getElementById("modalContent");
  const bossVictory =
    state.expansions.includes("mguf_yn_returns") && state.levelPhase === "boss";
  const sceptrePath = `items/${SCEPTRE_IMG}`;
  const sceptreMarkup = imgStatus[sceptrePath]
    ? `<img src="${IMG_BASE}${sceptrePath}" class="victory-sceptre" alt="Scettro di M'Guf-yn">`
    : `<div class="victory-sceptre victory-sceptre-emoji">👑</div>`;
  m.innerHTML = `<div class="modal-scroll">
    ${sceptreMarkup}
  ...
  bg.classList.remove("hidden");
  m.querySelector("#againBtn").onclick = () => {
    bg.classList.add("hidden");
    showSplash();
  };
}
...
function renderStats() {
  ...
  const canAssign =
    state.phase === "assign" && !state.animating && state.selectedDie !== null;
  ...
  const lootActive =
    !!state.lootDieActive &&
    (state.phase === "roll" || state.phase === "assign") &&
    !state.animating &&
    state.loot > 0;
  ...
  const targetable = {
    speed:
      (canAssign && !speedTaken && !rangeTaken) ||
      (lootActive && lootFree("speed")),
  ...
  function statBox(id, label, val, diceBonus, cardBonus) {
    const cls = targetableAbility[id]
  ...
  const showCards =
    state.expansions &&
    state.expansions.includes("item_cards") &&
  ...
  if (showCards) {
    ...
    cardsBox.className =
      "stat cards-stat" + (anyUsable ? "" : " cards-stat-dim");
  ...
  row.querySelectorAll(".stat[data-stat]").forEach((el) => {
    ...
    if (targetable[st] || targetableAbility[st])
      el.onclick = () =>
        state.lootDieActive ? incrementLoot(st) : assignTo(st);
  });
}
...
function renderDice() {
  const row = document.getElementById("diceRow"),
    ...
    label = document.getElementById("trayLabel");
  row.innerHTML = "";
  assignRow.innerHTML = "";
  if (state.phase === "roll") {
    ...
    if (
      state.class === "paladin" &&
      !state.abilityUsed.paladin &&
      state.prevDice.length === 3
    ) {
    ...
    state.dice.forEach((d, i) => {
      ...
      const flameSelected =
        flameSelectable && state.flameFateSelected.includes(i);
      el.className =
        "die" +
        (d.target ? " used" : "") +
        (state.selectedDie === i ? " selected" : "") +
        (flameSelected ? " flame-selected" : "") +
        (d.isD12 ? " d12" : "");
      el.textContent = d.value;
      el.onclick = flameSelectable
        ? () => toggleFlameFateDie(i)
        : () => selectDie(i);
      row.appendChild(el);
    });
    ...
    if (state.flameFateMode) {
      ...
      confirmBtn.className =
        "assign-btn" + (state.flameFateSelected.length ? "" : " disabled");
      ...
      cancelBtn.className = "assign-btn";
    ...
    if (
      state.class === "wizard" &&
      !state.abilityUsed.wizard &&
      !state.dice.some((d) => d.target)
    ) {
      const b = document.createElement("button");
      b.className = "assign-btn";
      b.textContent = "🔮 Rilancia tutti (Mago)";
    ...
    if (
      ...
      const b = document.createElement("button");
      b.className = "assign-btn";
      b.textContent = "🪓 Rilancia tutti (Barbaro)";
    ...
    if (
      ...
      if (a === b2 && b2 === c) {
        const b = document.createElement("button");
        b.className = "assign-btn";
        b.textContent = "✨ Tris! Aumenta tutti di 2 (Chierico)";
    ...
    if (
      state.class === "thief" &&
      !state.abilityUsed.thief &&
      !state.dice.some((d) => d.target)
    ) {
      const b = document.createElement("button");
      b.className = "assign-btn";
      b.textContent = "🗝️ Tutti i dadi +1 (Ladro)";
    ...
    if (
      state.class === "necromancer" &&
      !state.abilityUsed.necromancer &&
      state.hp > 1
    ) {
      const target = necromancerTarget();
      const b = document.createElement("button");
...
function renderGrid() {
  ...
  for (let y = 0; y < GRID; y++) {
    for (let x = 0; x < GRID; x++) {
      ...
      if ((isEntry || isExit) && !isWall && !isChestCell)
        cell.classList.add("stair");

      const isPlayer = state.player.x === x && state.player.y === y;
      const monster = state.monsters.find(
        (m) => m.alive && m.x === x && m.y === y,
      );

      if (state.phase === "act" && !isWall) {
        const info = cellInfo(x, y);
        if (info.type === "move") cell.classList.add("reachable");
        if ((info.type === "monster" || info.type === "chest") && info.canHit)
          cell.classList.add("attackable");
      }
      if (state.selectedMonster && monster === state.selectedMonster)
        cell.classList.add("selected-monster");
      if (state.selectedChest && isChestCell)
        cell.classList.add("selected-monster");
      if (state.animAttacker && monster === state.animAttacker)
...
function renderInspect() {
  ...
  if (state.monsters.length) {
    ...
    const sel =
      state.selectedMonster && state.selectedMonster.alive
        ? state.selectedMonster
        : null;
    const info =
      sel && state.phase === "act" ? cellInfo(sel.x, sel.y) : { canHit: false };
  ...
  if (state.chest && !state.chest.opened) {
    const chestInfo =
      state.selectedChest && state.phase === "act"
        ? cellInfo(state.chest.x, state.chest.y)
        : { canHit: false };
    if (chestInfo.canHit) {
...
function render() {
  ...
  const enemyName = isBossPhase
    ? BOSS_TABLE[state.level].name
    : MONSTER_TABLE[state.level].name;
  const levelText = isBossPhase
    ? `Livello ${state.level} · Boss`
    : `Livello ${state.level} di ${MAX_LEVEL}`;
  document.getElementById("levelBanner").textContent =
    `${levelText} — ${isBossPhase ? "👑 " : ""}${enemyName} · ${clsInfo.icon} ${clsInfo.name}`;
  renderStats();
  renderDice();
  renderGrid();
  renderInspect();
  ...
  const awaitingLootConfirm =
    state.phase === "assign" &&
    state.dice.length > 0 &&
    state.dice.every((d) => d.target) &&
    state.loot > 0;
  rollBtn.classList.toggle("hidden", state.phase !== "roll");
  endTurnBtn.classList.toggle(
    "hidden",
    state.phase !== "act" && !awaitingLootConfirm,
  );
...
function renderSplash() {
  ...
  if (save) {
    ...
    const savedLevelLabel =
      save.levelPhase === "boss" ? `${save.level} · Boss` : `${save.level}`;
  ...
  if (heroBox) {
    ...
    heroBox.innerHTML = tokenMarkup(
      cls.icon,
      cls.img ? `classes/${cls.img}` : null,
      "splash-hero-token",
    );
  }
  ...
  if (vanillaRank > 0)
    lines.push(`🏆 Vanilla — Livello ${rankToLabel(vanillaRank)}`);
  Object.keys(EXPANSIONS).forEach((id) => {
    const rank = loadRecord(id);
    if (rank > 0)
      lines.push(`🏆 ${EXPANSIONS[id].name} — Livello ${rankToLabel(rank)}`);
  });
  if (lines.length) {
...
function showExpansionSelect() {
  ...
  const render2 = () => {
    ...
    Object.entries(EXPANSIONS).forEach(([id, e]) => {
      ...
      b.onclick = () => {
        setExpansionActive(id, !isActive);
        render2();
      };
      list.appendChild(b);
    });
    m.querySelector("#expCloseBtn").onclick = () => {
      bg.classList.add("hidden");
    };
  };
  render2();
...
function showInstructions() {
  ...
  `;
  m.querySelector("#instrCloseBtn").onclick = () => {
    bg.classList.add("hidden");
  };
  bg.classList.remove("hidden");
}
...
document.getElementById("expansionsBtn").onclick = () => {
  showExpansionSelect();
};
document.getElementById("infoBtnSplash").onclick = () => {
  showInstructions();
};
...
document.getElementById("continueBtn2").onclick = () => {
  ...
  if (!state.itemBonus)
    state.itemBonus = { speed: 0, atk: 0, def: 0, range: 0 };
  if (state.halveDamageThisTurn === undefined)
    state.halveDamageThisTurn = false;
  if (state.monsterSpeedOverride === undefined)
    state.monsterSpeedOverride = null;
  if (state.skipMonstersThisTurn === undefined)
    state.skipMonstersThisTurn = false;
...
document.getElementById("resetBtn").onclick = () => {
  if (
    !confirm(
      "Cancellare la partita in corso e tutti i record? L'azione non è reversibile.",
    )
  )
    return;
  window.__ocdCache.save = null;
